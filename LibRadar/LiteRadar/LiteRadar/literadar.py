...
if __name__ == '__main__':
    ...
    libraries_packages = []
    for element in res:
        libraries_names. append(element.get("Library"))
        libraries_packages. append(element.get("Package"))
    directory_store = '/Users/dass/Tools/Results/librerias/processing_info/'
    appname = os.path.basename(iron_apk_path)[:-4]
    with open(directory_store+"names/"+appname+'.csv','w') as f:
        f.write( ','.join(libraries_names) )
        f.close()
    with open(directory_store+"packages/"+appname+'.csv','w') as ff:
        ff.write( ','.join(libraries_packages) )
        ff.close()
    print(json.dumps(res, indent=4, sort_keys=True, encoding='utf-8', ensure_ascii=False))